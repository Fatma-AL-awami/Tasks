...
class three_Fragment : Fragment() {
    ...
    companion object {
        fun newInstance(s: String, s1: String): two_Fragment {
            return two_Fragment()
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        taskListViewModel.task3ListLiveData.observe(
            viewLifecycleOwner,
            Observer { task ->
                task?.let {
    ...
    private inner class CrimeHolder(view: View) : RecyclerView.ViewHolder(view),
        ...
        val titleTextView: TextView = itemView.findViewById(R.id.Task3_Title)
        val detailsTextView: TextView = itemView.findViewById(R.id.Task3_details)
        val dateTextView: TextView = itemView.findViewById(R.id.Task3_date)
    ...
    private inner class CrimeAdapter(var tasks: List<Task>) :
        ListAdapter<Task, CrimeHolder>(TaskDiffutil()) {

        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CrimeHolder {
            //  val toast = Toast.makeText(context,(crimes.size).toString() , Toast.LENGTH_LONG)
            //toast.show()

            val view = layoutInflater.inflate(R.layout.tap_list_fragment, parent, false)
